 ...
 *
 * Three ways of finding it, in the order of how much somebody meant them:
 *
...
function serverCommand(extensionPath, configured, probes = {}) {
  ...
  const main = join(root, "packages", "lsp", "src", "main.ts");
  if (exists(main) && exists(join(root, "node_modules", "vscode-languageserver"))) {
    const found = version();
    const problem = isRecentEnough(found)
      ? undefined
      : `node ${NEEDS_NODE.major}.${NEEDS_NODE.minor} or newer runs the server from a checkout; ` +
        `${found ?? "no node"} was found on the PATH.`;
    return { command: "node", args: [main, "--stdio"], why: `the checkout at ${root}`, problem };
  }
