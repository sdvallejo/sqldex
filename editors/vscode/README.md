...
| **Findings as you type** — 48 rules over names, schema, queries and routines: a column that does not exist, an `INSERT` whose value count does not match, an `UPDATE` with no filter, a variable never read, a foreign key with no index | the Problems panel and the squiggles; <kbd>F8</kbd> walks them |
