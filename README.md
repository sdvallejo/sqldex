...
| Lint rules | 29 of them, in five groups — see below |
...
Twenty-nine of them, in five groups that say what a rule is *about* — which is what someone
...
| `query` | `insert-value-count`, `insert-unknown-column`, `unfiltered-write`, `join-without-condition`, `collation-mismatch`, `left-join-arithmetic`, `join-multiplies-aggregate`, `nullable-scalar-subquery` |
...
```
npm test                        # 410 tests, hand-written fixtures only
npm run typecheck
