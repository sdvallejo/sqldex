 ...
 *
 * `main.ts` is the file `bin` actually points at, and it runs the Node-version check before
 * anything here is imported — `@sqldex/core` uses `node:fs`'s `globSync`, which does not exist on a
 * Node old enough to fail that check, and an ES module's imports evaluate before its importer's own
 * code does regardless of where the `import` line sits in the file. Keeping that check in a file
 * with no imports of its own is what lets it run first.
 */
